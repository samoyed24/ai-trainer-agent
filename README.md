...
```python
from train_guard.config import SERVER_CONFIG, AGENT_CONFIG, METRICS_CONFIG
from train_guard.agent import TrainingAgent
...
```bash
# 通过 CLI 配置
train-guard-config set agent.upload_frequency batch --type str
train-guard-config set agent.upload_interval 10 --type int
```
...
### Q: 报错 `ModuleNotFoundError: No module named 'train_guard'` 怎么办？
