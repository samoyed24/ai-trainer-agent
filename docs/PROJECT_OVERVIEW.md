...
| 文件 | 用途 | 使用场景 |
|------|------|---------|
| `ai_trainer_agent/agent.py` | 训练Agent主体 | 核心业务逻辑 |
| `ai_trainer_agent/metrics_collector.py` | 指标采集 | 内部使用 |
| `ai_trainer_agent/uploader.py` | 指标上传 | 内部使用 |
| `ai_trainer_agent/config.py` | 配置管理 | 修改服务器地址、上传频率等 |
...
### 步骤2: 配置服务器
编辑 `ai_trainer_agent/config.py`，修改 `SERVER_CONFIG` 中的服务器地址（已预配置示例服务器）
...
### ai_trainer_agent/config.py 配置项
...
3. 阅读 `ai_trainer_agent/agent.py` 和 `ai_trainer_agent/metrics_collector.py` 源码

### 高级 (2小时+)
1. 阅读 [README.md](../README.md) "高级用法" 和 "配置详解" 部分
2. 阅读 `examples/example_training.py` 完整训练示例
3. 修改 `ai_trainer_agent/config.py` 实现自定义配置
...
### Q: 如何修改上传的目标服务器？
A: 修改 `ai_trainer_agent/config.py` 中的 `SERVER_CONFIG['url']`。
